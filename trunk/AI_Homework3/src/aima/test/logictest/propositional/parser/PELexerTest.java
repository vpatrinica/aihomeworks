...
import aima.logic.prop.infrastructure.PELexer;

public class PELexerTest extends TestCase {
	public void testLexBasicExpression() {
		PELexer pelexer = new PELexer();
		pelexer.setInput("(P)");
		assertEquals(new Token(LogicTokenTypes.LPAREN, "("), pelexer.nextToken());
		assertEquals(new Token(LogicTokenTypes.SYMBOL, "P"), pelexer.nextToken());
		assertEquals(new Token(LogicTokenTypes.RPAREN, ")"), pelexer.nextToken());
	...
	public void testLexNotExpression() {
		...
		assertEquals(new Token(LogicTokenTypes.LPAREN, "("), pelexer.nextToken());
		assertEquals(new Token(LogicTokenTypes.CONNECTOR, "NOT"), pelexer.nextToken());
	...
	public void testLexImpliesExpression() {
		...
		assertEquals(new Token(LogicTokenTypes.LPAREN, "("), pelexer.nextToken());
		assertEquals(new Token(LogicTokenTypes.SYMBOL, "P"), pelexer.nextToken());
		assertEquals(new Token(LogicTokenTypes.CONNECTOR, "=>"), pelexer.nextToken());
	}
	...
	public void testLexBiCOnditionalExpression() {
		...
		assertEquals(new Token(LogicTokenTypes.LPAREN, "("), pelexer.nextToken());
		assertEquals(new Token(LogicTokenTypes.SYMBOL, "B11"), pelexer.nextToken());
		assertEquals(new Token(LogicTokenTypes.CONNECTOR, "<=>"), pelexer.nextToken());
	}
}
